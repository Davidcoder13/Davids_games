(() => {
  ...
  function paintRoads() {
    const roadW = CONFIG.cellSize * 0.34;

    // Base links: soft shadow then light road top.
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';

    for (const pass of [0, 1]) {
      ctx.strokeStyle = pass === 0 ? '#cdbf9f' : '#f8f4e8';
      ctx.lineWidth = pass === 0 ? roadW + 4 : roadW;
      for (const k of state.roadCells) {
        const { x, y } = parseKey(k);
    ...
    // Bridges get a turquoise top coat.
    ctx.strokeStyle = '#6fc1b6';
    ctx.lineWidth = roadW * 0.88;
    ...
    // Smooth corners and rounded nodes (roads do not fill whole cells).
    for (const k of state.roadCells) {
      const { x, y } = parseKey(k);
      const center = gridToWorld(x, y);
      const ns = roadNeighborCells(x, y);

      ctx.fillStyle = state.bridgeCells.has(k) ? '#6fc1b6' : '#f8f4e8';
      ctx.beginPath();
    ...
    // remembered intersections visual ring
    for (const k of state.intersections) {
      const pCell = parseKey(k);
      const p = gridToWorld(pCell.x, pCell.y);
